(function ($) {
        ...
        $(this).each(function () {
            ...
            $(window).bind('scroll.sl resize.sl load.sl', function (e) {
                ...
                if ($fixed_item.height() < $win.height() &&
                    scroll_top > (pos.top - 1000) &&
                    $fixed_item.height() < $win.height()) {
                    !$fixed_item.hasClass('fixed') && $fixed_item.addClass('fixed');
                    var max_top = Math.min(45, $(document).height() - $fixed_item.height() - scroll_top - extra_offset);
                    $fixed_item.css('top', max_top + 'px');
                }
                else if (scroll_top <= (pos.top - 1000) &&
    ...
    if (window.location.pathname == '/admin/'){
        var elem = document.getElementById('content-main').getElementsByTagName('div');
        if (elem[0].getElementsByTagName('th')[2].innerHTML == 'Группа статейs'){
            elem[0].getElementsByTagName('tr')[2].remove()
        }
    }
    var elem = document.getElementById('content-main').getElementsByTagName('div');
    var elem2 = elem[3].getElementsByTagName('tr')[0]
    if (elem2){
        elem[0].getElementsByTagName('tbody')[0].append(elem2)
        elem[3].remove()
    }

    if (window.location.pathname == '/admin/'){
        elem[0].getElementsByTagName('a')[0].text = 'Работа со статьями'
        elem[1].getElementsByTagName('a')[0].text = 'Главная страница'
        elem[2].getElementsByTagName('a')[0].text = 'Медиафайлы'
        elem[2].getElementsByTagName('th')[0].innerHTML = 'Материалы'
        for(i=0;i < 9; i++){
            if(elem[0].getElementsByTagName('th')[i].innerHTML == 'Страницы'){
                elem[0].getElementsByTagName('th')[i].innerHTML = 'Статьи'
            }
        }
    }
